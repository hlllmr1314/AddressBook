...
-(void)dealloc
{
    ...
    [histroyTypedWords release];
    
    [filteredArray removeAllObjects];
    [filteredArray release];
    
    [allUnitArray removeAllObjects];
    [allUnitArray release];
    
    [mutipleChoosedMap removeAllObjects];
    [mutipleChoosedMap release];
    
    [_correspondGlob release],_correspondGlob = nil;
    
    [letterDict removeAllObjects];
    [letterDict release],letterDict = nil;
...
-(void)filterInDatabaseByABC:(NSString *)searchText
{
    ...
    while ([rs next]){
        ...
        for (NSString *key in KEYINDB) {
            if ([[rs resultDictionary] objectForKey:key] != nil) {
                matchCount +=1;
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) cell = [[[SearchResultCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SearchResult"] autorelease];
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PersonDetailViewController *personDetail = [[[PersonDetailViewController alloc] init] autorelease];
